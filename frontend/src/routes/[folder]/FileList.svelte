<script lang="ts">
    ...
    import {DownloadRequestSchema, type FileView, ListFilesRequestSchema} from "$lib/grpc/gen/folder/v1/files_pb";
    ...
    import {EncryptedBlobsSchema, type EncryptedVault} from "$lib/grpc/gen/folder/v1/encryption_pb";
    ...
    async function downloadFile(file: DecryptedFileView, onProgress: (percent: number) => void) {
        ...
        for await (const chunk of useDownload.call(create(DownloadRequestSchema, {
            file: file.id,
            folder: id
        }))) {
            if (chunk.blob && chunk.blob.part) {
                ...
                const percent = Math.min(100, Math.round((receivedLength / totalSize) * 100));
                onProgress(percent);
            }
        ...
        try {
            decryptedData = await decryptBlob(key, create(EncryptedBlobsSchema, {
                meta: vault!!,
...
{:else if files}
    <div class="flex flex-col gap-1">
        <span>{m["files.files"]({ count: files.size, max: limitsStore.data?.maxFilesPerFolder ?? '...' })}</span>
